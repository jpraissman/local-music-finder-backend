...
def send_event_email(event):
  ...
  try:
    ...
    body = f'<p>Thanks for creating an event with The Local Music Finder! Your event has been posted and is available for all users to see.</p><p>To <strong>edit</strong> or <strong>delete</strong> your event, <a href={event_edit_link}>click here</a> and paste in your Event ID: <strong>{event.event_id}</strong>.</p><p><span style="text-decoration: underline;">Your Event Details:</span><br /><em>Venue Name: </em>{event.venue.venue_name}<br /> <em>Venue Address: </em>{event.venue.address}<br /><em>Band Name:</em>&nbsp;{event.band_name}<br /><em>Band Type:</em> {event.band_type}<br /><em>Band Genres:</em> {", ".join(event.genres)}<br /><em>Event Date: </em>{event.event_date.strftime("%B %d, %Y")}<br /> <em>Start Time: </em>{event.start_time.strftime("%I:%M %p")}</p><br /><p>Sincerely,<br />The Local Music Finder</p>' 
...
def send_duplicate_event_email(events):
  ...
  try:
    ...
    count = 1
    for event in events:
      body += f"<p>Event {count}: At <strong>{event.venue.venue_name}</strong> with the address <strong>{event.venue.address}</strong>.</p>"
      count += 1
